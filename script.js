...
function render(filter='all'){grid.innerHTML=products.filter(p=>filter==='all'||(p.category||p.type)===filter).map((p,i)=>`<article class="card"><div class="pic"><img src="assets/catalog/${p.img}" alt="${p.name} — ${p.category || p.type}, 100 ДИВАНОФЪ" loading="lazy"></div><div class="info"><span class="type">${p.type}</span><h3>${p.name}</h3><div class="price">${p.price}</div><div class="spec"><b>Размеры</b><span>${p.size}</span></div><div class="spec"><b>Цвета</b><span>${p.colors}</span></div><button class="more" data-i="${products.indexOf(p)}">Подробнее и заказать →</button></div></article>`).join('');
